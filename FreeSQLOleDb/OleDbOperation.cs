...
{
   internal abstract class OleDbOperation : Operation
   {
      public OleDbOperation(OleDbConnection connection)
         : this(connection, null)
      { }

      public OleDbOperation(OleDbConnection connection, OleDbTransaction transaction)
      ...
      {
         return (OleDbField)property.GetCustomAttributes(true)
            .FirstOrDefault(a => a.GetType() == typeof(OleDbField) && ((OleDbField)a).TableIndex == tableIndex);
      }
      ...
      internal protected OleDbCommand GetLastIDCommand(Table t)
      {
         string query = string.Format("SELECT @@IDENTITY AS last_id;", t.TableName);
