...
{
   internal class SelectTopOleDbOperation<T> : OleDbOperation
   {
      // local variables
      ...
      public SelectTopOleDbOperation(int rows, string column, bool desc, OleDbConnection connection, OleDbTransaction transaction)
      ...
      public SelectTopOleDbOperation(int rows, string[] columns, bool[] desc, OleDbConnection connection, OleDbTransaction transaction)
         : base(connection, transaction)
      {
         wRows = rows;
         wColumns = columns;
      ...
      private OleDbCommand GetSelectTopCommand(int topRows, string[] columns, bool[] descs)
      {
         ...
         string where = (!tabAttr[0].VirtualDelete) ? "" : string.Format("WHERE (t{0}.ativo = ?) ", tabAttr[0].Index);
