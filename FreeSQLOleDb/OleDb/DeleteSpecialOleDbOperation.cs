...
namespace FreeSQL.Database.OleDb
{
   internal class DeleteSpecialOleDbOperation<T> : OleDbOperation
   {
      ...
      public DeleteSpecialOleDbOperation(string column, object value, bool ignoreVirtual, OleDbConnection connection, OleDbTransaction transaction)
      ...
      public DeleteSpecialOleDbOperation(string[] columns, object[] values, bool ignoreVirtual, OleDbConnection connection, OleDbTransaction transaction)
         : base(connection, transaction)
      {
         wColumns = columns;
         wValues = values;
      ...
      private OleDbCommand GetDeleteSpecialCommand(string[] columns, object[] values)
      {
         ...
         if (!wIgnore && t.VirtualDelete) cmd.Parameters.Insert(0, new OleDbParameter("@ativo", OleDbType.Boolean) { Value = false });
         return cmd;
      }
   }
}